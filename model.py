...
feat_cols = ['clonesize', 'honeybee', 'osmia', 'MinOfUpperTRange', 
       ...
       'fruitmass', 'seeds']
X_test_rf_df = pd.read_csv(curr_path + "/csv_files/X_test_rf_df.csv", index_col= 0)
rf_final = joblib.load(curr_path + "/joblib_files/randomforest_blueberry_final_model.joblib")
...
def predict_yield(attributes: np.ndarray):
    ...
    #                 feat_cols, 
    #                 show=False, 
    #                 matplotlib=True).savefig(curr_path + "/images_saved/force_plot_custom.png",
    #                                          bbox_inches = 'tight')

    # image = Image.open(curr_path + '/images_saved/force_plot_custom.png')
